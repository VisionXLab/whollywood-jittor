...
import math
import numpy as np
from jdet.models.boxes.box_ops import mintheta_obb, distance2bbox, boxes_x0y0x1y1_to_xywh
...
class P2RSubnetHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(P2RSubnetHead, self).__init__()
    ...
    def loss(self,
        ...
        pos_bbox_targets, pos_angle_targets = \
            jt.split(flatten_bbox_targets[pos_inds], [4, 1], dim=1)
        ...
        if num_pos > 0:
            ...
            pos_decoded_bbox_targets = distance2bbox(pos_points, pos_bbox_targets)
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(targets)):
            ...
            gt_bboxes = targets[img_id]['rboxes']
            gt_labels = targets[img_id]['labels']
            ...
            targets[img_id]['rboxes'] = det_bboxes
        return targets
    ...
    def _get_bboxes_single(self,
        ...
        """
        gt_pos = (gt_bboxes[:, 0:2] / self.strides[0] * scale_factor).long()
        gt_idx = gt_pos[:, 1] * int(img_shape[1] / self.strides[0]) + gt_pos[:, 0]

        cls_score, bbox_pred = cls_score_list[0], bbox_pred_list[0]
        bbox_pred = bbox_pred.permute(1, 2, 0).reshape(-1, 4)[gt_idx] * self.strides[0]
        cls_score = cls_score.permute(1, 2, 0).reshape(-1, self.num_classes)[gt_idx]
        priors = mlvl_priors[0][gt_idx]
        bboxes = boxes_x0y0x1y1_to_xywh(distance2bbox(priors, bbox_pred, max_shape=img_shape))
        bboxes[:, 0:2] = gt_bboxes[:, 0:2]
        bboxes[:, 2:4] = bboxes[:, 2:4] / scale_factor
        gt_point = jt.logical_and(gt_bboxes[:, 2] <= self.point_dummy,
                                    gt_bboxes[:, 3] <= self.point_dummy)
        bboxes = jt.cat((bboxes, jt.zeros_like(bboxes[:, 0:1])), -1)
        bboxes[gt_point.logical_not(), 2:] = gt_bboxes[gt_point.logical_not(), 2:]
        return bboxes.detach()
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, points, regress_ranges,
        ...
        return labels, jt.concat([bbox_targets, theta_targets], dim=1)
